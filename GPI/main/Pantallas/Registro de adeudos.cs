...
{
    public partial class Registro_de_adeudos : Form
    {
        public Registro_de_adeudos()
        {
            InitializeComponent();
        }
        ...
        private void Registro_de_adeudos_Load(object sender, EventArgs e)
        {
            timer1.Enabled = true;
            txtIDempleado.Focus();
        ...
        private void txtCLV_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        {
            Adeudo_pendiente adeudo = new Adeudo_pendiente();
            double monto = 0;
        ...
        private void btnAdeudoPendiente_Click(object sender, EventArgs e)
        {
            if (DGVlistA.RowCount==0)
            {
                MessageBox.Show("No hay articulo(s) registrado como adeudo.");  
            }
            else
            {
                for (int i = 0; i < DGVlistA.RowCount; ++i)
                {
                    ...
                    cmd.CommandText = "insert into AdeudoPendiente values(" + txtIDempleado.Text + "," + clave + ",'" + lblTimer.Text + "'," + cant + ", " + precio + ",'false','true','false')";
                    cmd.CommandType = CommandType.Text;
                    cmd.Connection = con.conecta;
                    con.abrirbd();
                    cmd.ExecuteNonQuery();
